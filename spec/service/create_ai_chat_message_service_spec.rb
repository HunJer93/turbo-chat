...
describe CreateAiChatMessageService, type: :service do
  ...
  let(:stubbed_answer) { "This answer is a stubbed answer" }

  before do
    # stub the call to the external service to create the llm instance
  ...
  context "when all parameters are present" do
    # Although is not necessary to provide both ai_chat_id and user_id, this is a tolerated case and user_id will be ignored
    let(:parameters) { { prompt:, ai_chat_id: ai_chat.id, user_id: user.id } }

    it "create a new AiMessage" do
  ...
  context 'when ai_chat_id and user_id are NOT provided' do
    ...
    it 'add specific error message' do
      service.call
      expect(service.errors[:ai_chat_id]).to include('the user_id is required')
    end
  end
